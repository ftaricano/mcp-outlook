...
(async () => {
  ...
  // --- EMAIL READ ---
  const listEmails = await run('list_emails limit:5', () =>
    tool('list_emails', { limit: 5, folder: 'inbox' })
  );
  ...
  console.error(
    `[info] parsed ${idMatches.length} candidate ids, firstLen=${firstEmailId?.length}`
  );

  await run('summarize_emails_batch limit:2', () =>
    tool('summarize_emails_batch', { limit: 2, priorityOnly: false })
  );
  ...
  // --- FOLDERS ---
  const listFolders = await run(
    'list_folders',
    () => tool('list_folders', { includeSubfolders: false, maxDepth: 1 }),
    { check: (t) => /Inbox|Caixa de Entrada|Archive/i.test(t) }
  );
  await run(
    'get_folder_stats inbox',
    () => tool('get_folder_stats', { folderId: 'inbox', includeSubfolders: false }),
  ...
  await run('advanced_search with dateFrom', () =>
  ...
  await run('advanced_search only hasAttachments (known InefficientFilter)', () =>
    tool('advanced_search', {
      hasAttachments: true,
      maxResults: 3,
      folder: 'inbox',
  ...
  const senderDomain = process.env.LIVE_SMOKE_SENDER_DOMAIN ?? 'example.com';
  await run('search_by_sender_domain', () =>
    tool('search_by_sender_domain', {
      domain: senderDomain,
      maxResults: 3,
  ...
  await run('search_by_attachment_type pdf', () =>
    tool('search_by_attachment_type', {
      fileTypes: ['pdf'],
      maxResults: 3,
      folder: 'inbox',
    })
  ...
  await run('search_by_size 1-10 MB', () =>
    tool('search_by_size', {
      minSizeMB: 1,
      maxSizeMB: 10,
      folder: 'inbox',
  ...
  await run('find_duplicate_emails subject', () =>
    tool('find_duplicate_emails', {
      criteria: 'subject',
      folder: 'inbox',
      maxResults: 20,
    })
  ...
  await run('cleanup_old_downloads dryRun', () =>
    tool('cleanup_old_downloads', { daysOld: 365, dryRun: true })
  );

  // --- WIZARDS (dry-run only) ---
  await run('email_cleanup_wizard dryRun', () =>
    tool('email_cleanup_wizard', {
      dryRun: true,
      olderThanDays: 365,
      maxEmails: 20,
    })
  );
  await run('organize_emails_by_rules dryRun', () =>
    tool('organize_emails_by_rules', {
