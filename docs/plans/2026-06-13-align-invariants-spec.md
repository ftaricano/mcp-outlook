...
3. **Extrair** `redactSecrets(message: string): string` num módulo pequeno (`src/utils/redactSecrets.ts`) cobrindo email, JWT/Bearer, base64/base64url ≥20, e atribuições nomeadas (`password|client_secret|access_token|api_key|secret|token = …`). Aplicá-lo em **todos os caminhos que carregam erro originado no Graph/runtime**, via o helper compartilhado `formatRedactedError(prefix, error)` (fonte única): `BaseHandler.formatError` (linha inteira, pois handlers interpolam `result.error` no `message`), `BaseHandler.redactError` nos resultados parciais de batch/folder/attachment (via `formatSuccess`), e o catch top-level do `CallToolRequestSchema` em `src/index.ts`. Assinatura de `formatError` inalterada. **Fora de escopo, por design:** os caminhos de **erro de validação de input** (`HandlerRegistry` args inválidos, `AttachmentValidator`) **não** passam por redact — são pré-Graph (não carregam secret), o validador não ecoa o valor cru, e o regex de token (base64 ≥20) **over-mascararia identificadores legítimos** como nomes de tool (`move_emails_to_folder`) e de arquivo. *(escopo refinado pelo loop de gate — ver Revisão pós-gate.)*
...
**Iteração 2 (Codex, BLOCK):** o catch top-level do `CallToolRequestSchema` (`index.ts`) devolvia `error.message` cru se `handleTool` lançasse → fechado extraindo `formatRedactedError` (fonte única) e usando-o no catch e no `formatError`.

**Iteração 3 (Codex sem token → fallback Claude Code Reviewer, APPROVE):** varredura final dos caminhos de erro. O reviewer apontou 2 caminhos de validação de input (`HandlerRegistry:43`, `EmailHandler` attachment) sem redact, porém safe (pré-Graph, sem secret). Tentei mascará-los e o teste provou que `redactSecrets` **over-mascara nomes de tool ≥20 chars** (`move_emails_to_folder` → `[token]`). Decisão: **não mascarar caminhos de validação de input** (sem ganho de segurança + corromperia identificadores) — registrado no escopo acima. Gate final: **APPROVE (fallback)**, 0 blockers.
