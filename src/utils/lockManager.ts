...
export class LockManager {
  ...
  public acquire(): void {
    if (fs.existsSync(this.lockFilePath)) {
      try {
        ...
        if ((error as any).code !== 'EEXIST') {
          ...
          if (error instanceof Error && error.message.startsWith('Server is already running')) {
            throw error;
          }
          console.error(`Error reading lockfile, assuming stale/corrupt: ${error}`);
          try {
            fs.unlinkSync(this.lockFilePath);
          } catch {
    ...
    try {
      ...
      throw new Error(
        `Failed to write lockfile at ${this.lockFilePath}: ${(error as Error).message}`
      );
    }
  }
