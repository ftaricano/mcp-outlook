...
export class AttachmentHandler extends BaseHandler {
  ...
  async handleDownloadAttachmentToFile(args: any): Promise<HandlerResult> {
    ...
    try {
      const result = await this.emailService.downloadAttachmentToFile(emailId, attachmentId, {
        targetDirectory,
  ...
  async handleDownloadAllAttachments(args: any): Promise<HandlerResult> {
    ...
    try {
      const result = await this.emailService.downloadAllAttachmentsFromEmail(emailId, {
        targetDirectory,
        overwrite,
        validateIntegrity,
        maxConcurrent,
      ...
      const successRate =
        result.totalFiles > 0
          ? ((result.successfulDownloads / result.totalFiles) * 100).toFixed(1)
          : '0';
