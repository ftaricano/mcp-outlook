...
export class BatchHandler extends BaseHandler {
  ...
  async handleBatchDeleteEmails(args: any): Promise<HandlerResult> {
    ...
    try {
      const results = await this.emailService.batchDeleteEmails(emailArray, {
        permanent,
        maxConcurrent,
      });
  ...
  async handleBatchMoveEmails(args: any): Promise<HandlerResult> {
    ...
    try {
      ...
      const results = await this.emailService.batchMoveEmails(emailArray, targetFolderId, {
        maxConcurrent,
      });
      ...
      results.forEach((downloadResult, index) => {
        ...
        if (downloadResult.success) {
          ...
          if (downloadResult.fileNames && downloadResult.fileNames.length > 0) {
            ...
            const moreFiles =
              downloadResult.fileNames.length > 3
                ? ` +${downloadResult.fileNames.length - 3} mais`
                : '';
